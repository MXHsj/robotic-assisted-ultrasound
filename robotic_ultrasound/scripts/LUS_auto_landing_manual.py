...
cmd_pos_pub = rospy.Publisher(
    'franka_cmd_pos', Float64MultiArray, queue_size=1)
cmd_acc_pub = rospy.Publisher(
    'franka_cmd_acc', Twist, queue_size=1)
    ...
    try:
        while True:
            ...
            isReachedTrans = True if \
                all([abs(err) < 0.002 for err in trans_error]) else False
            isReachedRot = True if \
                all([abs(err) < 0.03 for err in rot_error]) else False
