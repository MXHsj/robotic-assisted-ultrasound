...
def get_joint_pos():
    ...
    ee_sub = rospy.Subscriber(
        "franka_state_controller/franka_states", FrankaState, callback)
...
def callback(data):
    ...
    T_8_cam = np.array([[0.999, 0.0, 0.0, 0.0], [
                       0.0, -0.999, 0.0, -0.035], [0.0, 0.0, -0.999, 0.16], [0.0, 0.0, 0.0, 1.0]])
...
def Goal_corr(Radius_desired, pitch, yaw, x_obj, y_obj, z_obj, goal_x, goal_y, goal_z):
    ...
    if np.sqrt(goal_x*goal_x + goal_y*goal_y) > R:
        # radius correction
        Radius_desired = R - np.sqrt(x_obj*x_obj+y_obj*y_obj)
...
def calcGoal(Radius, pitch, yaw, x_obj, y_obj, z_obj):
    ...
    [goal_x, goal_y, goal_z] = Goal_corr(
        Radius, pitch, yaw, x_obj, y_obj, z_obj, goal_x, goal_y, goal_z)
...
def main():
    ...
    pygame.display.set_caption('Franka Emika Keyboard Teleop Console')
    ...
    while True:
        ...
        if keys[K_w]:
            ...
            if pitch > max_pitch:
                pitch = max_pitch
            [goal_x, goal_y, goal_z] = calcGoal(
                Radius, pitch, yaw, x_obj, y_obj, z_obj)
